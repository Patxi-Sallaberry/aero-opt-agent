...
Meilleure des **12 itérations** de la série `iterations` (12 réussies, 0 échouée), retenue sur l'objectif `maximize_Cl_Cd`.
...
| **Optimisé (itération 11)** | **0.04699** | **0.22540** | **4.80** |

**Gain de finesse : +39.3 %**
...
| `thickness` | 0.12 | **0.109693** unitless | -8.6 % | 0.08 … 0.2 |
| `camber` | 0.02 | **0.0242** unitless | +21.0 % | 0 … 0.09 |
| `span` | 80 | **80** mm | inchangé | 79 … 81 |
| `aoa` | 0 | **0** deg | inchangé | -2 … 12 |

![Section du profil](figures/profile_shape.svg)

## Pourquoi cette forme est meilleure

- **Cambrure accentuée** (0.0200 → 0.0242). La cambrure décale toute la courbe de portance : un profil cambré porte déjà à incidence nulle. Elle se paie en traînée de forme et en moment de tangage, d'où l'existence d'un optimum plutôt qu'une croissance sans fin.
- **Profil aminci** (0.1200 → 0.1097 d'épaisseur relative). Un profil plus fin perturbe moins l'écoulement et traîne moins. La contrepartie est structurelle — moins d'inertie, donc moins de rigidité — et un décrochage plus brutal, le bord d'attaque plus aigu supportant mal les fortes incidences.
...
- **Le compromis chiffré** : la portance gagne +29 % *et* la traînée baisse de 8 %. Les deux termes s'améliorent à la fois — cas favorable, qui tient ici à ce que la surface de référence suit la corde.
...
Exporté le 20/08/2026 à 10:57 UTC par `scripts/export_best.py`.
